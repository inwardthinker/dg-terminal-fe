...
export const PositionRowTable = React.memo(function PositionRowTable({
    ...
    return (
        ...
        >
            ...
            <div className="px-3 py-0 text-right text-secondary max-sm:hidden">
                <span
                    key={`current-${currentPriceCents}`}
                    className="inline-block live-value-flash px-1"
                >
                    {currentPriceCents}
                </span>
            </div>
            ...
            {/* P&L */}
            <div
                className={`py-0 text-right transition-colors duration-200 ${showStalePnl ? 'text-support opacity-60' : pnlTextClass(position.pnl)
                    }`}
                aria-live="polite"
                aria-atomic="true"
            >
            ...
            {/* P&L % */}
            <div
                className={`px-3 py-0 text-center transition-colors duration-200 ${showStalePnl ? 'text-support opacity-60' : pnlTextClass(position.pnlPct)
                    } max-sm:hidden`}
            >
                <span
                    key={`pnlpct-${pnlPctText}`}
                    className="inline-block live-value-flash px-1"
                >
                    {pnlPctText}
                </span>
            ...
            <div className="px-3 py-0 text-center">
                <Button variant="destructive" size="sm" onClick={handleClose} className="w-fit max-sm:text-[10px] max-sm:w-10"
                >
                    Close
                </Button>
            </div>
