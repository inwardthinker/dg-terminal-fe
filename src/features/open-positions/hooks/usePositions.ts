...
function normalizeIncomingPriceEvent(payload: unknown): PositionPriceEvent | null {
  ...
  return {
    ...
    no_of_shares: pickFirst(
      payload,
      ['no_of_shares', 'noOfShares', 'shares', 'quantity'],
      asOptionalNumber,
    ),
    avg_price: pickFirst(
      payload,
      ['avg_price', 'avgPrice', 'entry_price', 'entryPrice'],
      asOptionalNumber,
    ),
    current_price: pickFirst(
      payload,
      ['current_price', 'currentPrice', 'mark_price', 'markPrice'],
      asOptionalNumber,
    ),
    position_value: pickFirst(
      payload,
      ['position_value', 'positionValue', 'size', 'notional'],
      asOptionalNumber,
    ),
    pnl_amount: pickFirst(payload, ['pnl_amount', 'pnlAmount', 'pnl'], asOptionalNumber),
    pnl_percent: pickFirst(
      payload,
      ['pnl_percent', 'pnlPercent', 'pnl_pct', 'pnlPct'],
      asOptionalNumber,
    ),
  ...
  useEffect(() => {
    ...
    if (isLiveSocketMode) {
      if (realtimeOnly && !resolvedAddress) {
        setPositionsState(mockPositions.map((position) => ({ ...position, priceStale: true })))
        setLoading(false)
