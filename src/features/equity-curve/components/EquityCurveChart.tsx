...
function EquityCurveChartComponent({ data, width, loading, color }: EquityCurveChartProps) {
  ...
  const hoveredPoint = hoverIndex !== null ? computed.points[hoverIndex] : null

  const hoveredData = hoverIndex !== null ? data[hoverIndex] : null

  const prev = hoverIndex !== null && hoverIndex > 0 ? data[hoverIndex - 1] : null

  const change = prev && hoveredData ? ((hoveredData.value - prev.value) / prev.value) * 100 : 0
  ...
  return (
    <div className="relative w-full h-full">
      ...
      >
        ...
        {hoveredPoint && (
          <circle cx={hoveredPoint.x} cy={hoveredPoint.y} r={3} fill={strokeColor} />
        )}
      ...
      {hoveredPoint && hoveredData && (
        ...
        >
          ...
          {/* Balance */}
          <div className="text-t-2">${hoveredData.value.toFixed(0)}</div>
