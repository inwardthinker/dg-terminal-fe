...
export default function TerminalLoader({
    ...
    const setProgressSmooth = useCallback(
        (targetPct: number, stepDuration: number) => {
    ...
    const runStep = useCallback(
        (index: number) => {
            ...
            setLogEntries((prev) => [
                ...prev,
                { id: entryId, text: step.text, done: false, isLast },
            ])
        ...
        <div className='m-0 w-full overflow-hidden border border-[#2a2a22] bg-[#0d0d0a] font-jetbrains'>
            ...
            <div className='flex h-[280px]'>
                <div className='flex-1 overflow-hidden px-4 py-[18px]'>
                    {logEntries.map((entry) => (
                        ...
                        >
                            ...
                            <span
                                className={`flex-1 ${entry.isLast ? "text-pos" : "text-white"}`}
                            >
                                {entry.text}
                ...
                <div className='w-[180px] shrink-0 border-l border-[#2a2a22] px-[14px] py-[14px] text-[10px] tracking-widest text-[#666655] bg-black divide-y divide-line-c'>
                    <div className='pb-2'>
                        <div className='mb-2 text-[10px] tracking-widest text-t-3/75'>
                            TELEMETRY
                        </div>
                        ...
                        <div className='mb-1 flex justify-between'>
                            ...
                            <span className='font-bold text-[#c8c07a]'>
                    ...
                    <div className='pt-2 mb-4'>
                        <div className='mb-2 text-[9px] tracking-[0.2em] text-t-3/75'>
                            STREAM
                        </div>
                        {streamLines.map((line, i) => (
                            <div
                                key={i}
                                className={`text-[10px] leading-[1.6] tracking-[0.05em] transition-colors ${i === streamLines.length - 1 ? "text-t-3" : "text-t-3/50"
                                    }`}
                            >
                                {line}
                            </div>
            ...
            <div className='flex items-center gap-[10px] border-t border-[#2a2a22] px-[14px] py-[10px]'>
                <div className='flex h-3 flex-1 gap-[3px] border border-[#2a2a22] bg-[#1a1a14] px-[3px] py-[2px]'>
                    {Array.from({ length: SEGMENTS }).map((_, i) => (
                        <div
                            key={i}
                            className={`h-full flex-1 transition-colors ${i < filledSegments ? "bg-[#c8c07a]" : "bg-[#2a2a1a]"
                                }`}
                        />
                    ))}
                </div>
                <div className='min-w-9 text-right text-xs text-[#c8c07a]'>
                    {progress}%
                </div>
            </div>
        </div>
