  ...
  return (
    <div className="relative left-1/2 -ml-[50vw] w-screen max-w-[100vw] overflow-x-hidden border-y border-line-c bg-bg-0">
      <div className="mx-auto flex w-full max-w-[1400px] min-w-0 items-center justify-between px-sp5 py-[5px] sm:px-sp7">
            ...
            {items.map((item, index) => {
              ...
              return (
                <li key={index} className="flex items-center gap-1">
                  {item.href && !isLast ? (
                    <Link href={item.href} className="hover:text-t-2 transition-colors">
                      {item.label}
                    </Link>
                  ) : (
                    <span className={clsx(isLast && 'text-t-1 font-semibold')}>{item.label}</span>
                  )}
        ...
        <div className="text-secondary text-t-3/20! border border-line-c rounded-r2 px-sp3 py-sp1 max-sm:hidden">
          cmdP to close
        </div>
      </div>
    </div>
