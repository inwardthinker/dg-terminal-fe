...
export function IdentitySteps({ userName = null, onSkipToCalibrate }: IdentityStepsProps) {
  ...
  const suggestedHandle = normalizedUserName.replace(/[^a-z0-9_]/g, '').slice(0, MAX_HANDLE_LENGTH)
        ...
        <div className="px-6 pb-6 pt-4 bg-bg-3/10">
          ...
          <div className="flex flex-col">
            ...
            <p className="mt-1 text-xs tracking-tightest text-t-3/80 font-jetbrains">
              {handle.trim() ? `dgpredict.com/u/${handle.trim()}` : 'type a handle to see your URL'}
            </p>
            ...
            {suggestedHandle ? (
              <button
                ...
                className={`mt-2 inline-flex h-4 w-fit items-center border px-2 py-0 text-[10px] lowercase tracking-tightest font-jetbrains ${
            ...
            {availabilityState !== 'idle' ? (
              <p
                className={`mt-1 text-xs font-jetbrains ${
