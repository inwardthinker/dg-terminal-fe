...
export function ClosePositionModal({
  ...
  function handleConfirmClose() {
    ...
    submitTimerRef.current = setTimeout(() => {
      ...
      const closedPositions = isBulkClose ? positionsToClose : position ? [position] : []
  ...
  const totalLossAmount = losingPositions.reduce((sum, item) => sum + Math.abs(item.pnl), 0)
  ...
  const handleModeTabsKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const nextIndex =
      event.key === 'ArrowRight' ? (currentTabIndex + 1) % 2 : (currentTabIndex + 2 - 1) % 2
    ...
    return (
      <BaseModal variant="modal" onClose={handleClose} showClose={false}>
          ...
          {positionsToClose.map((item) => (
            ...
            >
              <div className="pr-2">
                ...
                {item.pnl < 0 && (
                  <span className="mt-0.5 block text-g-3!">⚠ Closing at a loss</span>
                )}
              </div>
              <span className={item.side === 'YES' ? 'text-pos' : 'text-neg'}>{item.side}</span>
              <span className="text-right">${Math.round(item.size).toLocaleString()}</span>
              <span className={`text-right ${item.pnl >= 0 ? 'text-pos' : 'text-neg'}`}>
                {item.pnl >= 0 ? '+' : '-'}${Math.abs(Math.round(item.pnl)).toLocaleString()}
              </span>
            </div>
          ))}
        </div>
        ...
        <div className="mt-4 rounded-lg bg-g-3/10 px-2 py-2">
          <SummaryLine
            label="Total closing"
            value={`$${Math.round(totalClosing).toLocaleString()}`}
          />
          <SummaryLine
    ...
    <BaseModal variant="modal" onClose={handleClose} showClose={false}>
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-secondary text-t-3!">{marketName}</p>
        </div>

        <div className="flex items-center gap-2">
          <span
            className={`rounded-xl px-4 py-1 text-secondary ${
              displaySide === 'NO'
        ...
        {['full', 'partial'].map((m, index) => (
          <Button
            ...
            className={`
              ...
              ${mode === m ? 'bg-g-3/10 text-g-3' : 'bg-transparent text-[14px] text-t-3/70!'}
            `}
          >
      ...
      {mode === 'partial' && (
        <div id="close-mode-panel-partial" role="tabpanel" aria-labelledby="close-mode-tab-partial">
          <div className="mt-3 flex items-center justify-between">
            <span className="text-secondary text-t-3!">Close how much?</span>
            <div
              className="flex items-center text-secondary font-semibold! text-g-3!"
              aria-live="polite"
              aria-atomic="true"
            >
          ...
          <div className="mt-3 grid grid-cols-3 gap-2 text-center">
            <div
              className="text-secondary text-t-3! bg-g-3/10 rounded-md px-3 py-2"
              aria-live="polite"
              aria-atomic="true"
            >
              <span className="uppercase text-support font-semibold!">Closing</span>
              <div className="text-primary font-semibold!">$2,500</div>
            </div>
            <div className="text-secondary text-t-3! bg-g-3/10 rounded-md px-3 py-2">
              <span className="uppercase text-support font-semibold!">Proceeds</span>
              <div className="text-primary font-semibold! text-pos!">$2,758</div>
            </div>
            <div className="text-secondary text-t-3! bg-g-3/10 rounded-md px-3 py-2">
          ...
          <div className="mt-3 flex items-center justify-between">
            <div className="text-secondary text-t-3!">Remaining after close</div>
            <div
              className="flex items-center text-secondary font-semibold! text-t-1!"
              aria-live="polite"
              aria-atomic="true"
            >
              <span>$2,500</span>
      ...
      {mode === 'full' && (
        <div id="close-mode-panel-full" role="tabpanel" aria-labelledby="close-mode-tab-full">
          <div className="mt-4 overflow-hidden rounded-lg" aria-live="polite" aria-atomic="true">
            ...
            />
            <InfoRow label="Size to close" value={sizeToCloseShares} />
            <InfoRow label="You will receive" value={`$${proceeds.toFixed(0)}`} hideDivider />
          </div>
